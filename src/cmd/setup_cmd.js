    ...
    if(setupCircom) {
        for(const subproof of airout.subproofs) {
            ...
            for(const air of subproof.airs) {
                ...
                log.info("[Setup  Cmd]", `······ Checkong if air '${air.name}' needs a compressor`);
                ...
                const tmpVerifierName = "tmp/" + (await tmpName()) + ".circom";
                await fs.promises.writeFile(tmpVerifierName, verifierCircomTemplate, "utf8");

                const compileRecursiveCommand = `circom --O1 --r1cs --prime goldilocks -l node_modules/pil2-stark-js/circuits.gl tmp/${tmpVerifierName}.circom -o tmp`;
                await exec(compileRecursiveCommand);

                const tmpR1csFile = `tmp/${tmpVerifierName}.r1cs`;
                const tmpR1cs = await readR1cs(tmpR1csFile, {F: setupOptions.F});
                const { NUsed } = getCompressorConstraints(setupOptions.F, tmpR1cs, 18);
                let nBits = Math.log2(NUsed - 1) + 1;
                ...
                const {starkInfo: starkInfoRecursive1, constRoot: constRootRecursive1, pil: pilRecursive1 } = await genRecursiveSetup("recursive1", subproof.subproofId, air.airId, constRoot, [], starkInfo, starkStructs["recursive1"], 18, hasCompressor);
            ...
            const {starkInfo: starkInfoRecursive2, constRoot: constRootRecursive2, pil: pilRecursive2 } = await genRecursiveSetup("recursive2", subproof.subproofId, undefined, undefined, constRootsRecursives1.map(c => c.constRoot), starkInfoRecursives1[0], starkStructs["recursive2"], 18)
